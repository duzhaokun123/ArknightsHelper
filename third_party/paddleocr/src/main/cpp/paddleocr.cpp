...
str_to_cpu_mode(const std::string &cpu_mode) {
  ...
  auto index = cpu_mode_map.find(cpu_mode);
  if (index == cpu_mode_map.end()) {
    LOGE("cpu_mode not found %s", cpu_mode.c_str());
...
Java_com_baidu_paddle_lite_demo_ocr_OCRPredictorNative_getVersionN(JNIEnv *env, jobject thiz,
                                                                   jlong native_pointer) {
  auto version = ((ppredictor::OCR_PPredictor*)native_pointer)->get_version();
  return cpp_string_to_jstring(env, version);
}